...
from PySide6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QLabel, QPushButton, QHBoxLayout, QLineEdit, QMessageBox, QFrame
...
class LearnWindow(QMainWindow):
    def __init__(self, model: VocabModel, parent=None):
        ...
        self.btn_return = QPushButton("返回主页面"); self.btn_return.setFixedSize(120,36); btn_row.addWidget(self.btn_return)
        layout.addLayout(btn_row)
        # display word
        self.word_label = QLabel("", alignment=Qt.AlignCenter); self.word_label.setFont(QFont("MiSans", 28, QFont.Bold)); layout.addWidget(self.word_label)
        # phase frame
        self.phase_frame = QFrame(); self.phase_layout = QVBoxLayout(self.phase_frame); layout.addWidget(self.phase_frame)
        # phase1 options
        opt_row = QHBoxLayout()
        self.opt_buttons = [QPushButton() for _ in range(4)]
        for b in self.opt_buttons:
            b.setFixedHeight(64); b.clicked.connect(self.on_choice); opt_row.addWidget(b)
        self.phase_layout.addLayout(opt_row)
        # phase2 know/unknow
        know_row = QHBoxLayout(); know_row.addStretch()
        self.know_btn = QPushButton("认识"); self.unknow_btn = QPushButton("不认识")
        self.know_btn.setFixedSize(120,40); self.unknow_btn.setFixedSize(120,40)
        know_row.addWidget(self.know_btn); know_row.addWidget(self.unknow_btn); know_row.addStretch()
        self.phase_layout.addLayout(know_row)
        self.know_btn.clicked.connect(self.on_know); self.unknow_btn.clicked.connect(self.on_unknow)
        # phase3 cloze + input
        self.cloze_label = QLabel("", alignment=Qt.AlignCenter); self.cloze_label.setFont(QFont("MiSans", 20, QFont.Bold)); layout.addWidget(self.cloze_label)
        self.spell_input = QLineEdit(); self.spell_input.setFixedHeight(36); layout.addWidget(self.spell_input)
        submit_row = QHBoxLayout(); submit_row.addStretch()
        self.submit_btn = QPushButton("提交"); self.idk_btn = QPushButton("我不会")
        self.submit_btn.setFixedSize(120,40); self.idk_btn.setFixedSize(120,40)
        submit_row.addWidget(self.submit_btn); submit_row.addWidget(self.idk_btn); submit_row.addStretch()
        layout.addLayout(submit_row)
        self.submit_btn.clicked.connect(self.on_submit); self.idk_btn.clicked.connect(self.on_idk)
        # prepare queue
        self.queue = deque()
        self.current = None
        # wire return
        self.btn_return.clicked.connect(self.close)
        # when opened, start automatically using model settings
        self._prepare_queue_and_start()

    def _prepare_queue_and_start(self):
        count = self.model.settings.get("learn_count", 10)
        pool = [w for w in self.model.words]  # include all; cycling will move stages
        if not pool:
            QMessageBox.information(self, "提示", "词库为空")
            return
        # initial order: sort by stage desc so higher stage get earlier
        ...
        for st in sorted(stages.keys(), reverse=True):
            grp = stages[st]; random.shuffle(grp)
    ...
    def _hide_all(self):
        ...
        self.know_btn.hide(); self.unknow_btn.hide(); self.cloze_label.hide(); self.spell_input.hide(); self.submit_btn.hide(); self.idk_btn.hide()
    ...
    def _enter_phase2(self, item):
        for b in self.opt_buttons: b.hide()
        self.know_btn.show(); self.unknow_btn.show()
        self.cloze_label.hide(); self.spell_input.hide(); self.submit_btn.hide(); self.idk_btn.hide()
        self.word_label.setText(item.word)

    def _enter_phase3(self, item):
        for b in self.opt_buttons: b.hide()
        self.know_btn.hide(); self.unknow_btn.hide()
        self.cloze_label.show(); self.spell_input.show(); self.submit_btn.show(); self.idk_btn.show()
    ...
    def on_choice(self):
        ...
        # update stage based on correctness
        ...
        # append to tail and save progress, then show next
    ...
    def on_unknow(self):
        ...
        self.queue.append(self.current)
        self.model.save_progress()
        # rotate until find a word with stage==1 or exhausted
        size = len(self.queue); rotated = 0
        while rotated < size:
            if len(self.queue) == 0: break
            if getattr(self.queue[0], "stage", 1) == 1:
                break
            self.queue.append(self.queue.popleft()); rotated += 1
