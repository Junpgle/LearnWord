...
try {
    ...
    Write-Host "      LearnWord Build Tool (v2.0 Slim)    " -ForegroundColor Cyan
    Write-Host "==========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    # 1. Build Main Program (PySide6 based)
    Write-Host "[1/3] Building Main Program (LearnWord)..." -ForegroundColor Yellow
    $buildCmd1 = "pyinstaller --noconfirm --onedir --windowed --clean --name `"LearnWord`" --icon `"icon.ico`" --add-data `"MiSans.ttf;.`" --add-data `"Animation;Animation`" main.py"
    cmd /c $buildCmd1
    ...
    # 【核心修改】强制排除所有 PySide6 和 Qt 相关库
    # 既然换用了 Tkinter，这些库对于 Updater 来说就是死重
